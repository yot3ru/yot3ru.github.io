...
- First entry begins as a clean Ink field at true zero. A Bone liquid surface then rises through the exact LUFI mark while two Cinnabar currents move only inside the completed area. A short pulse from the mark's circular counter seals the finished form before the page opens. Critical first-view readiness limits progress while a smoothed timeline controls its pace: cold entry resolves in roughly 4.5–5.5 seconds, warm reloads in 2.0–2.8 seconds, and internal navigation never replays it. Reduced motion uses a short flat fill, and a 6.5-second fail-safe always releases the page.
