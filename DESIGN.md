...
- First entry begins as a clean Ink field at true zero. Two Bone liquid fields rise independently through the exact LUFI mark, with restrained Cinnabar currents showing their different surface tension. The central Cinnabar point gathers last, then releases one quiet echo before the finished mark dissolves into the already-painted hero. The sequence uses compositor transforms and opacity instead of per-frame path updates. Cold entry resolves within roughly 2.5 seconds, warm reloads in roughly 1.5 seconds, and internal navigation never replays it. Reduced motion uses a short flat reveal, and a 5.2-second fail-safe releases the page with a fade.
